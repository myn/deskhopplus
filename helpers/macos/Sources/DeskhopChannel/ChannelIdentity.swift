...
public enum ChannelIdentity {
    /* Normal mode: pid.codes/1209/C000. Two channels. */
    public static let vendorID = Int(DH_CHANNEL_VENDOR_ID)
    public static let productID = Int(DH_CHANNEL_PRODUCT_ID)

    /* Config mode has one helper channel alongside its config API and disk. */
    ...
    /*
     * Match normal channels by page and usage. Config-mode discovery also
     * observes usage 0x10 for presence, but opens only the channel at 0x20.
     */
