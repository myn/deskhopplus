...
let copyWatchTests: [(String, () throws -> Void)] = [
    ("an unchanged count is ignored", {
        ...
        Check.equal(w.looked(at: 7, foundText: true), .ignore, "a stale count was acted on")
    }),

    ("a readable copy is taken on the first look", {
        var w = CopyWatch(changeCount: 7)
        Check.equal(w.looked(at: 8, foundText: true), .take(afterPolls: 1), "not taken")
        Check.equal(w.looked(at: 8, foundText: true), .ignore, "taken twice")
    }),

    ("a copy caught mid-write is waited for, not dropped", {
        var w = CopyWatch(changeCount: 7)
        /* changeCount has moved but the application has not written yet. */
        Check.equal(w.looked(at: 8, foundText: false), .wait, "an unfinished copy was dropped")
        Check.equal(w.looked(at: 8, foundText: false), .wait, "gave up too early")
        Check.equal(w.looked(at: 8, foundText: true), .take(afterPolls: 3),
    ...
    ("waiting is bounded, so an image or a file is let go", {
        ...
        for poll in 1..<CopyWatch.readAttempts {
            Check.equal(w.looked(at: 8, foundText: false), .wait, "gave up at poll \(poll)")
        }
        Check.equal(w.looked(at: 8, foundText: false), .giveUp, "waited past the bound")
        Check.equal(w.looked(at: 8, foundText: false), .ignore, "kept re-reading after giving up")
    }),

    ("a newer copy while waiting restarts the wait", {
        var w = CopyWatch(changeCount: 7)
        Check.equal(w.looked(at: 8, foundText: false), .wait, "not waiting")
        Check.equal(w.looked(at: 8, foundText: false), .wait, "not waiting")
        /* The user copied again. The new one gets the full budget, not what
           was left of the old one's. */
        Check.equal(w.looked(at: 9, foundText: false), .wait, "the newer copy inherited the wait")
        Check.equal(w.looked(at: 9, foundText: true), .take(afterPolls: 2), "not taken")
    }),

    ("this helper's own write is never read back as a local copy", {
        var w = CopyWatch(changeCount: 7)
        w.wrote(changeCount: 8)
        Check.equal(w.looked(at: 8, foundText: true), .ignore,
    ...
    ("a write while waiting cancels the wait", {
        ...
        Check.equal(w.looked(at: 9, foundText: true), .ignore, "the write was read back")
    }),
]
