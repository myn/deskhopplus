...
function fieldsFromLayout(layout) {
  ...
  if (overlap.some(v => v < 0)) return {refused:'There is a gap between the computers. Put them edge to edge.'};
  if (overlap.every(v => v > 0)) return {refused:'The computers overlap. Put them edge to edge.'};
  if (overlap.every(v => v === 0)) return {refused:'The computers touch only at a corner. Put an edge against an edge.'};
  ...
  for (const [i, o] of layout.outputs.entries()) {
    ...
    if (ms.length > 1) {
      ...
      if (chain === borders[i]) return {refused:'Put the main monitor next to the other computer.'};
    }
  ...
  if (pairs.length > 4) return {refused:`This layout needs ${pairs.length} segments; the board holds 4.`};
...
// A valid drop writes each changed field through the path a typed value takes,
// so it reaches the board at once and Save keeps its meaning. A refused drop
// shows its reason and touches nothing.
async function dropLabel(gesture) {
  ...
  for (const [key, value] of Object.entries(result.fields)) {
    ...
    if (String(getValue(element)) === String(value)) continue;
    element.value = value;
...
// Pointer glue: the block follows the pointer, then snaps to the half-box grid
// on release. A press is ignored while the last drop is still writing, so two
// drops never interleave their writes.
let dropping = null;
document.getElementById('layout').addEventListener('pointerdown', event => {
  const handle = event.target.closest('.layout-handle');
  if (!handle || event.button !== 0 || !event.isPrimary || dropping) return;
  event.preventDefault();
  const group = handle.parentNode, scale = handle.ownerSVGElement.getScreenCTM().a;
  const delta = e => [e.clientX-event.clientX, e.clientY-event.clientY].map(v => v/scale);
  const follow = e => group.setAttribute('transform', `translate(${delta(e).join(' ')})`);
  const drop = e => {
    window.removeEventListener('pointermove', follow); window.removeEventListener('pointerup', drop);
    group.removeAttribute('transform');
    const [dx, dy] = delta(e).map(v => Math.round(v/50)/2);
    dropping = dropLabel({output:handle.dataset.output, dx, dy}).finally(() => {dropping = null;});
  };
